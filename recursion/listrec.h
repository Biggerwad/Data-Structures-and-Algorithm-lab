...

{
    ...
    // Case 1: Current node is 'b' → insert 'a' before it
    if (p->dataItem == 'b')
    {
        p = new ListNode<DT>('a', p);
        aBeforebSub(p->next->next); // Continue with original 'b'
        return;
    }

    // Case 2: Next node is 'b' → insert 'a' before it
    if (p->next != nullptr && p->next->dataItem == 'b')
    {
        ListNode<DT> *newItem = new ListNode<DT>('a', p->next);
        p->next = newItem;
        aBeforebSub(newItem->next->next); // Skip over inserted 'a'
    }
    else
    {
        aBeforebSub(p->next); // Continue normally
    }
